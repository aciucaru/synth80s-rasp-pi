...
namespace synth80s
{
  SawtoothOscillator::SawtoothOscillator(AudioContext* audioContext)
  : MelodicOscillator(audioContext)
  { }

  void SawtoothOscillator::process(float* inputBlock)
  {
    ...
    for (int i = 0; i < BLOCK_SIZE; i++)
    {
      ...
      ** which initial starts from 0.0f */
      this->outputBlock[i] = -2.0f * this->gain / M_PI * std::atan(1.0f / std::tan(M_PI * this->phase));
